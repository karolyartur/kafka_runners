...
parser = argparse.ArgumentParser(description="Kafka Annotator")
...
class KafkaAnnotate(KafkaRunner):
    '''Kafka Runner for rendering annotations for synthetic data from Blender scenes

    Incoming Kafka messages are validated against the render_output.schema.json or the render_job.schema.json and outgoing messages are validated against the render_output.schema.json
    ...
    def msg_to_command(self, msg):
        '''Convert incoming Kafka messages to a command for doing the annotation process with Blender
        '''
        ...
        try:
            ...
            validate(instance=msg_json, schema=self.render_output_schema, resolver=resolver)
        except JSONDecodeError:
            ...
            self.logger.warn('Message "{}" failed JSON schema validation (used schema: {})\nFallback to direct annotation!'.format(msg.value, os.path.join(self.schema_path,'render_output.schema.json')))
            try:
            ...
            # If the message is the output of the renderer
            msg_json = msg_json['renderJob']
    ...
    def make_response(self, in_msg, elapsed_time):
        '''Construct a response after the annotation is completed
        '''
        try:
            msg_json = in_msg
            if isinstance(msg_json, str):
            ...
            self.logger.warn('Message "{}" failed JSON schema validation (used schema: {})\nFallback to direct annotation!'.format(in_msg, os.path.join(self.schema_path,'render_output.schema.json')))
            try:
                resolver = RefResolver(base_uri='file://'+os.path.abspath(self.schema_path)+'/'+'render_job.schema.json', referrer=None)
                validate(instance=msg_json, schema=self.render_job_schema, resolver=resolver)
            except ValidationError:
...
def main():
    ...
    renderer = KafkaAnnotate(args.in_topic_name, args.out_topic_name, args.bootstrap_servers, args.blender, consumer_group_id=args.consumer_group_id, error_topic_name=args.error_topic_name, loglevel=logging.DEBUG if args.debug else logging.WARN)
